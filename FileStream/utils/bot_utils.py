...
async def check_group_membership(client, user_id):
    if not hasattr(Telegram, 'AUTH_GROUPS') or not Telegram.AUTH_GROUPS:
        return True
        
    for group_id in Telegram.AUTH_GROUPS:
        try:
...
async def verify_user(bot, message):
    ...
    if await is_user_banned(message):
       return False
    group_status = await check_group_membership(bot, message.from_user.id)
    print("group_status", group_status)
    if group_status == "banned":
        await message.reply_text(
            text="You are banned from the main group. Contact admin.",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True
        )
        return False
    elif group_status == "restricted":
        await message.reply_text(
            text="You are restricted in the main group. Contact admin.",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True
        )
        return False
    elif group_status is False:
        await message.reply_text(
            text="You must be a member of the main group to use this bot.",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True
        )
        return False
